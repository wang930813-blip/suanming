...
$trade_order_id = $_REQUEST['WIDout_trade_no'];//商户网站内部ID，此处time()是演示数据
$trade_no = $_REQUEST['trade_no'];//
$appid              = '201906155170';//虎皮椒APPID
$appsecret          = 'd4f14b8985665c0e2af10d817ca8e82c';//账户，
...
$data=array(
    ...
    'wap_url'   => '',//网站域名，H5支付必填
    'wap_name'   => '',//网站域名，或者名字，必填，长度32或以内 H5支付必填
    'total_fee' => $_REQUEST['WIDtotal_fee'],//人民币，单位精确到分(测试账户只支持0.1元内付款)   $_REQUEST['WIDtotal_fee']
    'title'     => $_REQUEST['WIDsubject'], //必须的，订单标题，长度32或以内
    'time'      => time(),//必须的，当前时间戳，根据此字段判断订单请求是否已超时，防止第三方攻击服务器
    'notify_url'=>  '/hpj/notify.php', //必须的，支付成功异步回调接口
    'return_url'=> '/?ct=pay&ac=notify_yzf&out_trade_no='.$trade_order_id.'&trade_no='.$trade_no.'',//必须的，支付成功后的跳转地址
    'callback_url'=>$_SERVER['HTTP_HOST'],//必须的，支付发起地址（未支付或支付失败，系统会会跳到这个地址让用户修改支付信息）
