function checkForm() {
  try {
	...
	var name1 = document.login.name1.value;
	var name2 = document.login.name2.value;
	var name3 = document.login.name3.value;
	var name4 = document.login.name4.value;
	if (name1 === "") {
		layer.msg("请输入男方姓氏！");
  ...
  if (name2 === "") {
		layer.msg("请输入男方名字！");
		// $('.lunpan_box').css('display','none');
		document.login.name2.focus();
		return false;
	}
  if (name3 === "") {
		layer.msg("请输入女方姓氏！");
		// $('.lunpan_box').css('display','none');
		document.login.name3.focus();
		return false;
	}
  if (name4 === "") {
		layer.msg("请输入女方名字！");
		// $('.lunpan_box').css('display','none');
		document.login.name4.focus();
		return false;
	}

  var username = name1+''+name2;
	var girlusername = name3+''+name4;
  document.login.username.value=username
  document.login.girl_username.value=girlusername
...
// 构建格式化后的时间字符串 // ${hours}:${minutes}:${seconds}
const formattedTime = `${year}-${month}-${day}`;

	var b_input = formattedTime;
	var b_input1 = formattedTime;
  ...
  return setTimeout(function(){ return true},800)
  } catch (error) {
    console.log(error)
    return false
  }
