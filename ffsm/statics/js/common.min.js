requirejs.config({
	...
	ajaxForm_list.length && require(["ffsm.ajaxForm", "layer"], function() {
		...
		$(".J_ajax_submit_btn").on("click", function(e) {
			...
			if ($("#agreeInput").length && !$("#agreeInput").is(":checked")) return layer.msg("閹劍婀崥灞惧壈娑撴眽闂呮劗閸楀繗"), !1;
			if (lock) return !1;
			...
			"" !== btn.data("callback") && void 0 != btn.data("callback") && (callback = eval(btn.data("callback"))), isBrowserMsie && form.find("[placeholder]").each(function() {
				...
				beforeSubmit: function(e, a, o) {
					var t = btn.text();
					formArr = e, btn.text(t + "娑擄拷...").prop("disabled", !0).addClass("disabled"), layer_load = layer.load(0, {
						shade: !1
					});
				...
				success: function(e, a, o, t) {
					...
					var s = btn.text();
					btn.removeClass("disabled").text(s.replace("娑擄拷...", "")).parent().find("span").remove(), layer.close(layer_load), callback(e, btn), setTimeout(function() {
						lock = !1
					}, 2e3)
				}
			})
	...
	// 纭畾鏃ユ湡鐩存帴鎻愪氦
	// $(document).on('click', '.lcalendar_finish', function() {
		
	// 	if(document.login.username.value) {
	// 		$(".J_ajax_submit_btnsub").click();
	// 	}
	// });
