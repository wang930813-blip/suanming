 ...
 */
    ...
    try {
        ...
        file_put_contents(dirname(__FILE__).'/h5pay_back_error.log', date('Y-m-d H:i:s')." 签名验证失败: ".$e->getMessage()."\n", FILE_APPEND);
        die('sign error');
    }

    file_put_contents('h5pay_result.txt', json_encode($result));
    ...
    if ($mysqli->connect_error) {
        file_put_contents(dirname(__FILE__).'/h5pay_back_error.log', date('Y-m-d H:i:s')." 数据库连接失败: ".$mysqli->connect_error."\n", FILE_APPEND);
        die('db error');
    }
    ...
    if($result['return_code']=='SUCCESS' && $result['result_code']=='SUCCESS' && !empty($oid)){
        ...
        if ($order_result && $order_row = $order_result->fetch_assoc()) {
            ...
            if ($total_fee < $expected_amount) {
                file_put_contents(dirname(__FILE__).'/h5pay_back_error.log', 
            ...
            if ($order_row['status'] != 1) {
                ...
                if ($mysqli->query($sql)) {
                    file_put_contents(dirname(__FILE__).'/h5pay_back_success.log', 
                        date('Y-m-d H:i:s')." 订单{$oid} H5支付成功 交易号:{$transaction_id}\n", FILE_APPEND);
                } else {
                    file_put_contents(dirname(__FILE__).'/h5pay_back_error.log', 
