	...
	common.payRemind = function() {
		...
		ajaxForm.length && require(["ffsm-layerJS", "css!ffsm-layerCSS"], function() {
			...
			$(".J_ajax_submit_btn").on("click", function(e) {
				...
				if (inputPrivacyProtocol.length && !inputPrivacyProtocol.is(":checked")) return layer.msg("鎮ㄦ湭鍚屾剰涓汉闅愮鍗忚"), !1;
				...
				"" !== btn.data("callback") && void 0 != btn.data("callback") && (callback = eval(btn.data("callback"))), formDataArr = form.serializeArray(), $.ajax({
					...
					beforeSend: function() {
						var e = btn.text();
						btn.text(e + "涓�...").prop("disabled", !0).addClass("disabled"), layerLoad = layer.load(0, {
							shade: !1
						})
					},
					success: function(e, t, a, o) {
						...
						var r = btn.text();
						btn.removeClass("disabled").removeAttr("disabled").text(r.replace("涓�...", "")).parent().find("span").remove(), layer.close(layerLoad), callback(e, btn), setTimeout(function() {
							ajaxLock = !1
						}, 2e3)
					}
				})
		...
		if (e.length) {
			...
			/iphone|ipod|ipad/i.test(navigator.appVersion) && /MicroMessenger/i.test(t) && document.body.addEventListener("focusout", function() {
				...
				t(a.val()) && o.slideDown(), a.on("input propertychange", function() {
					...
					if (!t(a.val())) return void layer.msg("璇疯緭鍏ユ纭殑鎵嬫満鍙�");
					var e = $(this),
						o = $(this).data("action");
					if (!$(this).attr("disabled")) {
						var n = 60;
						e.attr("disabled", !0).html("60s鍚庨噸鑾�");
						var i = setInterval(function() {
							n--, n > 0 ? e.html(n + "s鍚庨噸鑾�") : (clearInterval(i), e.removeAttr("disabled").html("鑾峰彇楠岃瘉鐮�"))
						}, 1e3);
						$.ajax({
							url: o,
							data: {
							...
							dataType: "json",
							success: function(t) {
								if (t.code <= 0) return layer.msg(t.msg), clearInterval(i), e.removeAttr("disabled").html("鑾峰彇楠岃瘉鐮�"), !1;
								layer.msg("楠岃瘉鐮佸彂閫佹垚鍔�")
							}
						})
					}
					...
					if (l.length && !l.is(":checked")) return layer.msg("鎮ㄦ湭鍚屾剰涓汉闅愮鍗忚"), !1;
					if (n) return !1;
					n = !0;
					var d = function(e, t) {
							var a = e.msg,
					...
					o = r.serializeArray(), $.ajax({
						...
						beforeSend: function() {
							var t = i.text();
							i.text(t + "涓�...").prop("disabled", !0).addClass("disabled"), e = layer.load(0, {
								shade: !1
							})
						},
						success: function(t, a, r, s) {
							...
							var f = i.text();
							i.removeClass("disabled").text(f.replace("涓�...", "")).parent().find("span").remove(), layer.close(e), d(t, i), setTimeout(function() {
								n = !1
							}, 2e3)
						}
					})
		...
		var e = $("#evaluateBox");
		if (e.length) {
			var t = ["鐗瑰埆涓嶆弧鎰�", "涓嶅お婊℃剰", "涓€鑸�", "杩樼畻婊℃剰", "闈炲父婊℃剰"];
		...
		if ($("#publicFeedbackScroll").length) {
			...
			i.length > 0 && i.on("click", function() {
				require(["ffsm-layerJS", "css!ffsm-layerCSS"], function() {
					var e = r.val();
					s ? layer.msg("鎮ㄥ凡缁忔彁浜よ繃鍙嶉锛岀煭鏃堕棿涓嶈兘鍐嶆彁浜や簡鍝︼紒") : e ? (s = 1, r.val(""), layer.msg("鎻愪氦鎴愬姛锛岃阿璋㈡偍鐨勫弽棣堬紒", {
						icon: 1,
						time: 3e3
					})) : layer.msg("璇疯緭鍏ョ暀瑷€鍐呭锛�")
				})
			})
		}
		...
		$(".J_copyOrdersn").length && require(["ffsm-clipboard", "ffsm-layerJS", "css!ffsm-layerCSS"], function(e) {
			...
			t.on("success", function(e) {
				layer.msg("澶嶅埗鎴愬姛"), e.clearSelection()
			}), t.on("error", function(e) {
				layer.msg("璇烽暱鎸夊鍒惰鍗曞彿")
			})
		})
		...
		function e(e) {
			...
			var t = e.type,
				a = "";
			switch (e.name = e.name.replace("(鏂�)", ""), t) {
			case "input":
				a = '<div class="m_fp_formtype">\n                                <div class="name"><span>'.concat(e.name, '</span></div>\n                                <div class="input">\n                                    <input type="text" name="data[').concat(e.field, ']" value="" placeholder="璇疯緭鍏�" />\n                                </div>\n                            </div>');
				break;
			case "date":
				var o = Math.random().toString(36).substr(2);
				a = '<div class="m_fp_formtype">\n                            <div class="name"><span>'.concat(e.name, '</span></div>\n                            <div class="datepicker">\n                                <input type="text" name="" class="J_resultDatepicker" id="datepicker_').concat(o, '" data-toid-date="dateInput_').concat(o, '" value="" placeholder="璇疯緭鍏�" />\n                                <input type="hidden" name="data[').concat(e.field, ']" id="dateInput_').concat(o, '">\n                            </div>\n                        </div>');
				break;
			case "select":
				var n = "";
