function checkForm() {
	...
	var b_input_val = document.getElementById('birthday').value;
	if (b_input=="" || b_input==null || b_input_val == "" || b_input_val == null || b_input_val == "请选择出生日期") {
		$('.lunpan_box').css('display','none');
		layer.msg("请输入生日！");
		document.login.h.focus();
		return false;
	}else{
	    var in_date = b_input.split("-");
	    if(b_input != "" && in_date.length == 3){
	        
	        var h_year = parseInt(in_date[0]);
	        var h_mouth = parseInt(in_date[1]);
	        var h_day = parseInt(in_date[2]);
	        //console.log(h_year);
	        //console.log(h_mouth);
	        //console.log(h_day);
	        var n_nowDate = new Date(); // 当前时间
            var n_mouth = n_nowDate.getMonth() + 1;
            var n_day = n_nowDate.getDate();
            var n_year = n_nowDate.getFullYear();
            //console.log(n_mouth);
            //console.log(n_day);
            //console.log(n_year);
            //判断年龄是否大于18
            var isOver18age = false;
            if(n_year - h_year < 18){
                isOver18age = true;
            }else if(n_year - h_year == 18){
                if(n_mouth < h_mouth){
                    isOver18age = true;
                }else if(n_mouth == h_mouth){
                    if(n_day < h_day){
                        isOver18age = true;
                    }
                }
	...
	try {
	    ...
	    if(ck01){
	        ...
	        if(ck02){
	            var sp01 = ck02.split(",");
	            setval = (parseInt(sp01[0])+1) + ",no";
	        }else{
	            setval = "1,no";
	        }
	        $.cookie('TK_HKL001', ck01, { expires: 365});
	        $.cookie(ck01, setval, { expires: 365});
	    }else{
	        var timestamp1 = new Date().getTime();
	        var keystr1 = 'GS' + timestamp1;
	        $.cookie('TK_HKL001', keystr1, { expires: 365});
	        $.cookie(keystr1, '1,no', { expires: 365});
	    }
