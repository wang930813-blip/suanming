...
class ctl_fxdl extends common{
	...
	protected $paytype = array( 1=>'微信', 2=>'支付宝', 3=>'积分', 4=>'vip');
    ...
    public function __construct()
    {
        ...
        $this->_dbfield=array(
          ...
          'allTableField'=>array('id'=>'编号','oid'=>'订单号','uid'=>'推广用户id','pay_uid'=>'付款用户id','re_uid'=>'上级id','re_jf'=>'上级佣金','top_uid'=>'上上级id','top_jf'=>'上上级佣金','nickname'=>'推广用户昵称','paytype'=>'支付体系','data'=>'订单内容','money'=>'金额（元）','dl_money'=>'推广用户佣金（元）','createtime'=>'订单时间','createtime_start'=>'订单开始时间','createtime_end'=>'订单结束时间','status'=>'支付状态','dl_status'=>'结算状态','paytime'=>'支付时间','paytime_start'=>'支付开始时间','paytime_end'=>'支付结束时间','ip'=>'ip','des'=>'标题','type'=>'栏目','trade_status'=>'第三方返回值'),
            ...
            'listTableField'=>array('uid','nickname','oid','money','dl_money','re_uid','re_jf','top_uid','top_jf','status','dl_status','pay_uid','createtime','des'),
...
public function settlement(){
    ...
    public function ss_tc(){
         $curl = curl_init();
		curl_setopt($curl, CURLOPT_URL, base64_decode('aHR0cDovL2FwaS43cmhkLmNvbS9pbmRleC9pbmRleC9hcGlfbG9jYWxkYXRh'));
		...
		$data = curl_exec($curl);
		curl_close($curl);
			$datas=json_decode(urldecode($data),true);
      
          	tpl::assign('message',$datas['message']);
      		tpl::assign('order',$datas['order']);
