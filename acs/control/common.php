...
class common
{
    ...
    public function index(){
        ...
        $size = $config['page_size'] = 100; //每页显示多少
		...
		// 测算订单页面排除商城订单（订单号99开头）
		if($this->ct == 'ffsm_order' && $this->table == "ffsm_orders") {
			if($where=="" && $where_uid=="" && $where_dlid==""){
				$where=" where `oid` NOT LIKE '99%'";
				$whereog=" where og.`oid` NOT LIKE '99%'";
			}else{
				$where.=" and `oid` NOT LIKE '99%'";
				$whereog.=" and og.`oid` NOT LIKE '99%'";
			}
		}
  ...
  if($this->table == "ffsm_orders"){
        ...
        if($this->ct == 'fxdl'){
          ...
          $sql2 = "SELECT *,og.oid,max(og.status) as status FROM `".$this->table."` AS og LEFT JOIN `users` AS g ON g.uid = og.uid LEFT JOIN `ffsm_dl_fxjf` AS f ON f.oid = og.oid".$whereog.$where_uidog.$where_dlidog." and og.status = 1 GROUP BY  og.`oid` order by og.id  desc LIMIT {$offset},{$size}";
        }else{
          $sql1 = "SELECT count(".$this->_dbfield['mainKey'].") as  total,sum(money) as money_all,sum(if(status > 0, money, 0)) as money_cj,sum(if(status > 0, 1, 0)) as sum_cj,sum(dl_money) as dl_money  FROM (SELECT * FROM  `".$this->table."`".$where.$where_uid.$where_dlid." GROUP BY  `oid`) AA";
...
public function excel()
    {
		...
		tpl::assign('dl_syjf',$fields['dl_syjf']);
  		tpl::assign('integral',$fields['integral']);
        if(request('dosubmit',''))
        {
            $info=req::$posts;
    ...
    public function add()
    {
        ...
        if(request('dosubmit',''))
        {
			...
			if($this->table=="ffsm_dl_jftx")
			{
				if($info['tx_jf']>$fields['integral'])
				{
					cls_msgbox::show('系统提示','您的剩余积分不足，请重新操作',-1);
					exit();
				}
				else{
					db::query("UPDATE `users` SET `integral` = `integral`-'".$info['tx_jf']."' WHERE `uid` = '".cls_access::$accctl->uid."' limit 1");
               
					$info['tx_je']=round($info['tx_jf']/$tjex_txbl['config'],2);
				}
			}
			
			// 调用子类的_before_save方法（如果存在）
			if (method_exists($this, '_before_save')) {
				$this->_before_save($info);
			}
    ...
    public function edit()
    {
        ...
        if(request('dosubmit',''))
        {
          ...
          if($this->table=="ffsm_dl_jftx")
			{
				$dl_txzh_sc = db::get_one("Select * From `".$this->table."` where `id`='".$info['id']."' ");
				if($dl_txzh_sc['tx_sc']>=1)
				{
					$info['tx_sc']=$dl_txzh_sc['tx_sc'];
				}else{
					if($info['tx_sc']==2){
						db::query("UPDATE `users` SET `integral` = `integral`+'".$info['dl_jf']."' WHERE `uid` = '".cls_access::$accctl->uid."' limit 1");
					}
				}
        ...
        else
        {
			...
			// 清理position字段中的br标签，防止编辑时重复
			if (isset($data['position'])) {
				$data['position'] = str_replace(array('<br />', '<br/>', '<br>', '<BR />', '<BR/>', '<BR>'), "\n", $data['position']);
			}
