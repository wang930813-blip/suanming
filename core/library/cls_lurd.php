...
class cls_lurd
{
    ...
    public function get_pagination($url = '', $style = "ylmf-page")
    {
        ...
        if ($config['current_page'] > 1)
        {
            ...
            // 上一页
            $pages .= "
            <div class='dt-paging-button page-item'>
                <a href='{$config['url']}&{$config['page_name']}={$prev_page}' class='page-link next'>
                    <i class='previous'></i>
                </a>
            </div>
            ";
        ...
        for ($i = $config['current_page'] - 6; $i <= $config['current_page'] - 1; $i++)
        {
            ...
            $pages .= "
                <div class='dt-paging-button page-item'>
                    <a href='{$config['url']}&{$config['page_name']}=$_start' class='page-link next'>
                        $i
                    </a>
                </div>
        ...
        // 当前页
        $pages .= "
        <div class='dt-paging-button page-item active'>
            <a href='#' class='page-link' aria-controls='kt_ecommerce_products_table'>" . $config['current_page'] . "</a>
        </div>";
        // 后偏移
        ...
        if ($config['current_page'] != $config['count_page'])
        {
            ...
            $pages .= "
            <div class='dt-paging-button page-item'>
                <a href='{$config['url']}&{$config['page_name']}={$next_page}' class='page-link next'>
                    <i class='next'></i>
                </a>
            </div>";
            // 末页
        ...
        <div class="card-body pt-0">
            <div class="row">
            	<div id="" class="col-sm-12 col-md-5 d-flex align-items-center justify-content-center justify-content-md-start dt-toolbar">
            		<span>共 '.$config["count_page"].'页，'.$config["count_number"].' 记录
            	</div>
