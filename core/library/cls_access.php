...
// 🔴 修复：宝塔环境下 open_basedir 限制可能导致 session 无法写入 /tmp
// 将 session 保存路径设置为项目目录内，确保可写
$session_path = dirname(dirname(__DIR__)) . '/data/session';
if (!is_dir($session_path)) {
    @mkdir($session_path, 0777, true);
}
session_save_path($session_path);

// 设置 session cookie 参数，确保跨路径可用
...
class cls_access {
    ...
    protected function _put_logininfo(&$row, $keeptime = 0) {
        ...
        // 🔴 修复：立即保存 session 数据到磁盘，防止后续输出/跳转时 session 丢失
        session_write_close();
